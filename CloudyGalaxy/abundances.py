...
class ElementalAbundances:
    ...
    def __init__(self, logZ, F, scaling_method="dopita13_scaled"):
        ...
        self.mass = dict(
            H=1.0079,  # Mass of elements as from https://www.lenntech.com/periodic/mass/atomic-mass.htm
            He=4.0026,
            C=12.011,
            N=14.007,
            O=15.999,
    ...
    def set_depletion(self):
        ...
        for key in self.coef_A.keys():
            depl[key] = self.coef_B[key] + self.coef_A[key] * (
                self.F - self.coef_z[key]
            )
    ...
    def set_undepleted_abundance(self):
        ...
        def calc_CNO(logZ):
            ...
            oxy = np.array(
                [7.39, 7.50, 7.69, 7.99, 8.17, 8.39, 8.69, 8.80, 8.99, 9.17, 9.39]
            )
            nit = np.array(
                [
                    -6.61,
...
def set_coef_A(scaling_method):
    if scaling_method == "dopita13_scaled":
        ...
        coef_A = dict(
            C=-0.60,  # Not adopted from Jenkins09
...
def set_coef_B(scaling_method):
    if scaling_method == "dopita13_scaled":
        ...
        coef_B = dict(
            C=-0.30,  # Not adopted from Jenkins09
...
def set_coef_z(scaling_method):
    if scaling_method == "dopita13_scaled":
        ...
        coef_z = dict(
            C=0.5,  # Not adopted from Jenkins09
