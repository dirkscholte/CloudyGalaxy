...
class StellarSpectrum:
    ...
    def star_table_string(self, logZ, age=1e8):
        ...
        """
        if logZ > 0.5:
            return 'table star "{0}" age={1:.2e} logz={2:.2e}'.format(
                self.filename, age, 0.5
            )
        else:
            return 'table star "{0}" age={1:.2e} logz={2:.2e}'.format(
                self.filename, age, logZ
            )
    ...
    def load_spectrum(self, age, logZ):
        ...
        lines = f.readlines()
        header_lines = 11
        __, ndims, npars, par1, par2, nmod, nx, x, conv1, f_type, conv2 = lines[
            0:header_lines
        ]
...
def make_input_file(
    ...
    c_input = pc.CloudyInput(
        "{0}{1}".format(output_dir, full_model_name)
    )  # create input object for cloudy model

    c_input.set_star(
        SED=stellar_spectrum.star_table_string(logZ, age=age),
        SED_params="",
        lumi_unit="Q(H)",
    ...
    c_input.set_abund(
        ab_dict=elemental_abundances.abund, nograins=True
    )  # Set abundances
    c_input.set_cste_density(
        dens=np.log10(gas_stats.nH), ff=0.05
    )  # set constant hydrogen density
...
def make_emission_line_files(
    output_dir, model_name, logZs, logUs, xis, logtaus, Fs, n_attenuation=-1.3
):
    """
    ...
    for i in range(nlogZs):
        for j in range(nlogUs):
            for k in range(nxis):
                ...
                xi = xis[k]
                F = Fs[i, k]
                full_model_name = "{0}_{1:.3e}_{2:.3e}_{3:.3e}".format(
                    model_name, logZ, logU, xi
                )
                print("{0}{1}".format(output_dir, full_model_name))
                models = pc.load_models(
                    "{0}{1}".format(output_dir, full_model_name), read_grains=False
                )
                ...
                for l in range(nlogtaus):
                    logtau = logtaus[l]
                    print(
                        "PROCESSING: logZ = {0:06.4f}, logU = {1:06.4f}, xi = {2:06.4f}, logtau = {3:06.4f}.".format(
                            logZ, logU, xi, logtau
                        )
                    )

                    lambda_emission_lines = [
                        float(label[5:-1]) * 0.01 for label in model.emis_labels
                    ]  # Ang
                    emission_lines = [
                        model.get_emis_vol(ref=label) for label in model.emis_labels
                    ]  # erg/s
                    attenuated_emission_lines = emission_lines * transmission_function(
                        lambda_emission_lines, logtau, n=n_attenuation
                    )
                    emline_param_cube[i, j, k, l] = np.array([logZ, logU, xi, logtau])
                    emline_derived_param_cube[i, j, k, l] = np.array(
                        [
                            F,
                            model.abund["O"],
                            calc_log_dust(logtau),
                            calc_log_gas(logZ, xi, logtau),
                        ]
                    )
...
def make_unattenuated_emission_line_files(
    output_dir, model_name, logZs, logUs, xis, Fs
):
    """
    Reads Cloudy output files and extracts emission line fluxes. Writes to files along with line labels, line wavelengths and parameter values.
    :param output_dir: Output directory of the data files.
    ...
    for i in range(nlogZs):
        for j in range(nlogUs):
            for k in range(nxis):
                ...
                model = models[0]
                print(
                    "PROCESSING: logZ = {0:06.4f}, logU = {1:06.4f}, xi = {2:06.4f}.".format(
                        logZ, logU, xi
                    )
                )
    ...
    np.save(model_name + "_unattenuated_emission_line_labels.npy", emline_labels)
    np.save(model_name + "_unattenuated_emission_line_wavelengths.npy", emline_lambda)
    np.save(model_name + "_unattenuated_parameters_file.npy", emline_param_cube)
    np.save(
        model_name + "_unattenuated_derived_parameters_file.npy",
        emline_derived_param_cube,
    )
    np.save(
        model_name + "_unattenuated_emission_line_luminosity_file.npy",
        emline_luminosity_cube,
    )
